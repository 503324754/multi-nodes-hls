...
const demoConfig = clone(baseConfig, {
  ...
  entry: './demo/index',
  output: {
    filename: 'hls-demo2.js',
    chunkFilename: '[name].js',
    sourceMapFilename: 'hls-demo2.js.map',
...
function getConstantsForConfig(type) {
  // By default the "main" dists (yfhls.js & yfhls.min.js) are full-featured.
  return {
...
const multiConfig = [
  {
    ...
    output: {
      filename: 'yfhls.js',
      chunkFilename: '[name].js',
      sourceMapFilename: 'yfhls.js.map',
      path: path.resolve(__dirname, 'dist'),
      publicPath: '/dist/',
      library: 'YFHls',
      libraryTarget: 'umd',
